...
# Ensure database connection is available if needed later
try:
...
class LoginPage:
    def __init__(self, window):
        ...
        # ============================ Background Image ==========================
        # ========================================================================
        # Ensure 'images' folder exists with these files
        try:
        ...
        try:
            ...
            self.login = Button(self.lgn_button_label, text='LOGIN', font=("yu gothic ui", 13, "bold"), width=25, bd=0,
                                bg='#3047ff', cursor='hand2', activebackground='#3047ff', fg='white', command=self.validate)
            self.login.place(x=20, y=10)
        except:
            # Fallback button if image missing
            self.login = Button(self.lgn_frame, text='LOGIN', font=("yu gothic ui", 13, "bold"), width=25, bd=0,
                                bg='#3047ff', cursor='hand2', activebackground='#3047ff', fg='white', command=self.validate)
        ...
        # === REMOVED SIGN UP BUTTON SECTION HERE ===

    # Search for the 'validate' function and update it:
    def validate(self):
        user = self.username_entry.get()
        pwd = self.password_entry.get()
        
        if (user == 'admin' and pwd == 'admin') or (user != '' and pwd != ''):
            # === CHANGE START ===
            # self.window.destroy()  <-- DELETE THIS
            
            # Pass the existing window to the dashboard
            page_after_login.page_after_login(self.window) 
            # === CHANGE END ===
        else:
            ms.showerror('Oops!', 'Please enter a username and password')
